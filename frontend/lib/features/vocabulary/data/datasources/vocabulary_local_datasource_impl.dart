...
final class VocabularyLocalDataSourceImpl
  ...
  @override
  Future<void> update(VocabularyItemsCompanion companion) =>
      _db.update(_db.vocabularyItems).replace(companion);
  ...
  Future<int> getDueCount() async {
    final now = DateTime.now();
    final count = _db.vocabularyItems.id.count();
    final query = _db.selectOnly(_db.vocabularyItems)
      ..addColumns([count])
      ..where(_db.vocabularyItems.nextReviewAt.isNull() |
