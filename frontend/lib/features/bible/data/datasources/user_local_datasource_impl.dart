...
final class UserLocalDataSourceImpl implements UserLocalDataSource {
  ...
  Future<ReadingHistory?> getLastReadChapter() =>
      _db.getLastReadChapter();

  @override
  Future<List<ReadingHistory>> getRecentReadingHistory({
    int limit = 20,
  }) =>
  ...
  Future<int> addHighlight({
    ...
    int? endOffset,
      ...
      _db.into(_db.highlights).insert(
            HighlightsCompanion(
              bookId: Value(bookId),
              chapter: Value(chapter),
              verse: Value(verse),
